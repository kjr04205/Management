...
<div class="container">
	<div id="position_top">
		<h2 class="container_title">위치별 관리</h2>
		<button type="button" class="btn-1" id="insert">위치 등록</button>
	</div>
 	...
 	<div>
		...
		<div class="paging">
			<c:if test="${ph.showPrev}">
				<a href="<c:url value='/inventoryLocation${ph.sc.getQueryString(ph.beginPage-1)}'/>"><img src="resources/img/prev.png" /></a>
			</c:if>
			<c:forEach var="i" begin="${ph.beginPage }" end="${ph.endPage }">
				<a href="<c:url value='/inventoryLocation${ph.sc.getQueryString(i)}'/>">${i}</a>
			</c:forEach>
			<c:if test="${ph.showNext}">
				<a href="<c:url value='/inventoryLocation${ph.sc.getQueryString(ph.endPage+1)}'/>"><img src="resources/img/next.png" /></a>
			</c:if>
		</div>
		<div class="search-container">
            <form action="<c:url value="/inventoryLocation"/>" class="search-form" method="get">
                <select class="search-option" name="option">
                    <option value="R" ${ph.sc.option=='R' || ph.sc.option=='' ? "selected" : ""}>위치명</option>
                </select>
